...
class Renderer {
    ...
    */
    constructor(gl, scene, camera, context, inputHandler) {
        this.gl = gl;
        this.scene = scene;
        this.camera = camera;
    ...
    render() {
        ...
        for (var i = 0; i < this.scene.geometries.length; i++) {
            ...
            if(fog_mode == 0){
                ...
                geometry.shader = shader_fog;
                var eye = new Vector4(this.camera.eye.elements[0], this.camera.eye.elements[1], this.camera.eye.elements[2], 1.0);
                geometry.shader.setUniform("u_Eye", eye.elements);
                geometry.shader.setUniform("u_FogColor",fogColor.elements)
                geometry.shader.setUniform("u_FogDist",fogDist.elements);
                this.gl.clearColor(fogColor.elements[0], fogColor.elements[1], fogColor.elements[2], 1.0); // Color of Fog
                this.gl.enable(this.gl.DEPTH_TEST);
            }
